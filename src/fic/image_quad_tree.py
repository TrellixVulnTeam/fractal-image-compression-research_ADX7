...
class ImageQuadTree:
    ...
    def _find_the_best_source_block(self, source_blocks: List[torch.tensor]) -> Tuple[int, Transformation, float]:
        ...
        for source_idx, source in enumerate(source_blocks):
            if source.shape[-1] <= self._image.shape[-1]:
                continue
            transformation, loss = self._find_the_best_transformation(source, self._image)
            if loss < best_loss:
                best_loss = loss
    ...
    def _find_the_best_transformation(self, src: torch.tensor, dst: torch.tensor) -> Tuple[Transformation, float]:
